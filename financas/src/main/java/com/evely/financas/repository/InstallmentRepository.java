...
public interface InstallmentRepository extends JpaRepository <Installment, Integer> {
    ...
    @Query("SELECT SUM(i.amount) FROM Installment i WHERE i.payer.id = :userId " +
           "AND i.status = :status AND i.dueDate BETWEEN :inicio AND :fim")
    BigDecimal somarMensalPorUsuario(@Param("userId") Integer userId, 
